...
var imageCmd = &cobra.Command{
	...
	Long:  `show k8s resource image and container for workload like deployments/daemonsets/statefulsets/jobs/cronjobs`,
	RunE:  image,
}
...
func image(cmd *cobra.Command, args []string) error {
	...
	if flag, _ := cmd.Flags().GetBool("deployments"); flag {
		...
		if err != nil {
			fmt.Printf("list deployments error: %s\n", err.Error())
		}
	...
	if flag, _ := cmd.Flags().GetBool("daemonsets"); flag {
		...
		if err != nil {
			fmt.Printf("list daemonsets error: %s\n", err.Error())
		}
		rList = append(rList, deployList)
	}
	if flag, _ := cmd.Flags().GetBool("statefulsets"); flag {
		...
		if err != nil {
			fmt.Printf("list statefulsets error: %s\n", err.Error())
		}
		rList = append(rList, deployList)
	}
	...
	if flag, _ := cmd.Flags().GetBool("cronjobs"); flag {
		...
		if err != nil {
			fmt.Printf("list cronjobs error: %s\n", err.Error())
		}
		rList = append(rList, deployList)
	}
